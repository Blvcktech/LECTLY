...
# ──────────────────────────────────────────────
# Direct-to-R2 upload (presigned URL flow)
# ──────────────────────────────────────────────
...
async def presign_upload(
    ...
    """
    Step 1 of direct upload: Generate a presigned R2 URL.

    The browser calls this endpoint with file metadata. We validate
    limits, create a lecture record, and return a presigned PUT URL
    that the browser uses to upload directly to R2 — no file bytes
    pass through Railway.
    """
    ...
    from app.services.audio import LectureStatus
    ...
    lecture = {
        ...
        "filepath": f"r2://{file_key}",  # Mark as R2-stored
...
async def complete_upload(
    ...
    """
    Step 2 of direct upload: Confirm upload and start processing.

    Called by the browser after it finishes uploading to R2.
    Verifies the file exists in R2, then kicks off background processing.
    """
    from app.services.storage import file_exists, get_file_size

    body = await request.json()
    lecture_id = body.get("lecture_id")
    file_key = body.get("file_key")

    if not lecture_id or not file_key:
        raise HTTPException(status_code=400, detail="Missing lecture_id or file_key")

    # Verify the file was actually uploaded to R2
    if not file_exists(file_key):
        raise HTTPException(
            status_code=400,
            detail="File not found in storage. Upload may have failed — please try again.",
        )

    # Update the lecture record with actual file size from R2
    actual_size = get_file_size(file_key)
    if actual_size > 0:
        from app.database import update_lecture as update_lecture_db
        update_lecture_db(lecture_id, {"size_bytes": actual_size})

    print(f"[Lectly] R2 upload confirmed for {lecture_id} (key: {file_key}, size: {actual_size})")
